    ...
    en: {
        ...
        achievements: {
            ...
            rag: {
                ...
                description: '3,360+ indexed chunks, hybrid search (pgvector HNSW + BM25 + RRF fusion), neural reranking — self-hosted on AWS EC2 with CI/CD'
            },
            holders: {
        ...
        projectCards: {
            ...
            askdorosh: {
                ...
                description: 'Production RAG chatbot that indexes 34 real projects + CV (3,360+ chunks). Hybrid search (pgvector HNSW + BM25 GIN) with RRF fusion, Jina Reranker v3, DeepSeek LLM streaming via SSE. Self-hosted on AWS EC2 with Nginx, PM2, Let\'s Encrypt SSL, and GitHub Actions CI/CD. <a href="https://ask-about-dorosh.duckdns.org" target="_blank" style="color: var(--accent-color);">Try It Live</a>'
            },
            buddy: {
    ...
    uk: {
        ...
        achievements: {
            ...
            rag: {
                ...
                description: '3 360+ проіндексованих чанків, гібридний пошук (pgvector HNSW + BM25 + RRF), нейронний реранкінг — self-hosted на AWS EC2 з CI/CD'
            },
            holders: {
        ...
        projectCards: {
            ...
            askdorosh: {
                ...
                description: 'Продакшн RAG чат-бот, що індексує 34 реальні проекти + CV (3,360+ чанків). Гібридний пошук (pgvector HNSW + BM25 GIN) з RRF фузією, Jina Reranker v3, DeepSeek LLM стрімінг через SSE. Self-hosted на AWS EC2 з Nginx, PM2, Let\'s Encrypt SSL та GitHub Actions CI/CD. <a href="https://ask-about-dorosh.duckdns.org" target="_blank" style="color: var(--accent-color);">Спробувати наживо</a>'
            },
            buddy: {
    ...
    de: {
        ...
        achievements: {
            ...
            rag: {
                ...
                description: '3.360+ indexierte Chunks, hybride Suche (pgvector HNSW + BM25 + RRF), neuronales Reranking — self-hosted auf AWS EC2 mit CI/CD'
            },
            holders: {
        ...
        projectCards: {
            ...
            askdorosh: {
                ...
                description: 'Produktions-RAG-Chatbot, der 34 reale Projekte + CV indiziert (3.360+ Chunks). Hybridsuche (pgvector HNSW + BM25 GIN) mit RRF-Fusion, Jina Reranker v3, DeepSeek-LLM-Streaming via SSE. Self-hosted auf AWS EC2 mit Nginx, PM2, Let\'s Encrypt SSL und GitHub Actions CI/CD. <a href="https://ask-about-dorosh.duckdns.org" target="_blank" style="color: var(--accent-color);">Live testen</a>'
            },
            buddy: {
